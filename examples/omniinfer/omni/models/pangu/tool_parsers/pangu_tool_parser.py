...
class PanguToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.tool_call_start_token: str = "[unused11]"
        self.tool_call_end_token: str = "[unused12]"

        self.tool_call_regex = re.compile(
            r"\[unused11](.*?)\[unused12]|\[unused11](.*)", re.DOTALL)
        ...
        if self.tool_call_start_token not in model_output:
            ...
            try:
                ...
                for match in function_call_tuples:
                    tool_str = match[0] if match[0] else match[1]
                    tool_str = tool_str.strip()

                    # 仅支持管道风格: name|{...}
                    name_part, arg_part = tool_str.split("|", 1)
                    arguments = json.loads(arg_part.strip())
